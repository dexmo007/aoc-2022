...
min_x = min(x for path in stones for x, y in path)
max_x = max(x for path in stones for x, y in path)
grid = [[AIR for _ in range(max_x - min_x + 1)] for _ in range(max_y + 1)]
